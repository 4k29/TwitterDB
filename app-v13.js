(() => {
  ...
  const els = {
    type: $('typeFilter'), topic: $('topicFilter'), reply: $('replyFilter'), sort: $('sortOrder'), reset: $('resetFilters'),
    ...
    clearSelected: $('clearSelected'), deleteSelected: $('deleteSelected')
  };
  ...
  const TOPICS = [
    ['apple', 'Apple', /(apple|iphone|ipad|macbook|mac\b|ios\b|ipados|macos|watchos|visionos|airpods|airtag|homepod|apple watch|アップル|アイフォン|アイパッド)/i],
    ['nothing', 'Nothing', /(nothing|cmf|glyph|phone\s*\(?\d|ear\s*\(?|headphone\s*\(?)/i],
    ['photo', '写真・映像', /(写真|撮影|カメラ|レンズ|nikon|canon|eos|lightroom|raw\b|動画|映像|シネマティック|カラーグレーディング|カラグレ|ドローン|ジンバル)/i],
    ['dev', '開発・UI', /(github|プログラミング|開発|コード|サイト|web\b|html|css|javascript|typescript|react|next\.js|unity|ui\b|ux\b|デザイン|アプリ|データベース|db\b|api\b)/i],
    ['gadget', 'ガジェット', /(ガジェット|スマホ|タブレット|pc\b|パソコン|イヤホン|ヘッドホン|スピーカー|モニター|キーボード|マウス|充電|バッテリー|android|pixel|galaxy|fold|wearable|ウェアラブル|リング)/i],
    ['study', '勉強・進路', /(勉強|受験|大学|学校|授業|数学|物理|化学|英語|教員|教師|研究|進路|志望|入試|課題|宿題|模試|極限|微分|積分)/i],
    ['daily', '日常', /(今日|昨日|明日|朝|昼|夜|旅行|電車|ごはん|料理|買った|届いた|行った|帰る|帰宅|生活|日常)/i]
  ];
  ...
  function archiveRows() {
    const rows = [];
    for (const part of Object.values(window.YTD?.tweets || {})) if (Array.isArray(part)) rows.push(...part);
    return rows;
  }
  ...
  function extractMentions(text) {
    ...
    for (const match of String(text).matchAll(/@([A-Za-z0-9_]{1,15})\b/g)) {
      ...
      if (!seen.has(key)) { seen.add(key); values.push(label); keys.push(key); }
    }
    return { values, keys };
  }
  ...
  function detectTopic(text) {
    ...
    for (const [key, , pattern] of TOPICS) if (pattern.test(plain)) return key;
    return 'other';
  }

  function topicLabel(topic) {
    return Object.fromEntries(TOPICS.map(([key, label]) => [key, label]))[topic] || 'その他';
  }
  ...
  function normalizeRow(row) {
    ...
    if (replyTo) {
      ...
      if (!mentions.keys.includes(key)) { mentions.values.unshift(replyTo); mentions.keys.unshift(key); }
    }
    return {
      id, text, timestamp,
      dateLabel: timestamp ? dateFormatter.format(date) : '日時不明',
      timeLabel: timestamp ? timeFormatter.format(date) : '',
      mentions: mentions.values, mentionKeys: mentions.keys,
      likes: Number(tweet.favorite_count || 0), reposts: Number(tweet.retweet_count || 0),
      type: detectType(tweet, text), topic: detectTopic(text),
  ...
  function rebuildMentionFilter() {
    ...
    for (const tweet of allTweets) {
      ...
      tweet.mentionKeys.forEach((key, index) => { if (!labels.has(key)) labels.set(key, tweet.mentions[index]); });
    }
  ...
  function applyFilters({ rebuildMentions = false } = {}) {
    ...
    filtered = allTweets.filter((tweet) => {
      ...
      if (topic !== 'all' && tweet.topic !== topic) return false;
      if (mention !== 'all' && !tweet.mentionKeys.includes(mention)) return false;
      return true;
    });
    const sort = els.sort.value;
    filtered.sort((a, b) => sort === 'old' ? a.timestamp - b.timestamp : sort === 'likes' ? b.likes - a.likes || b.timestamp - a.timestamp : sort === 'reposts' ? b.reposts - a.reposts || b.timestamp - a.timestamp : b.timestamp - a.timestamp);
    const visibleIds = new Set(filtered.map((tweet) => tweet.id));
  ...
  function tweetRow(tweet) {
    ...
    return `<article class="tweet-row tweet-row-v5" data-id="${escapeHtml(tweet.id)}">
      ...
      <div class="tweet-body"><a class="tweet-text" href="${escapeHtml(tweet.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(tweet.text)}</a><div class="badges"><span class="badge">${typeLabel(tweet.type)}</span><span class="badge">${topicLabel(tweet.topic)}</span>${mentionBadges}</div></div>
  ...
  function updateBulkToolbar() {
    ...
    els.deleteSelected.disabled = count === 0 || writeInProgress;
    els.bulkActions.dataset.active = count ? 'true' : 'false';
  }

  function render() {
    els.resultCount.textContent = filtered.length.toLocaleString('ja-JP');
    els.rangeLabel.textContent = filtered.length ? `${filtered[filtered.length - 1].dateLabel} 〜 ${filtered[0].dateLabel}` : '条件に一致する投稿はありません';
  ...
  async function loadRemoteState({ renderAfter = false } = {}) {
    ...
    try {
      ...
      if (renderAfter) applyFilters({ rebuildMentions: true });
      return true;
    } catch (error) {
  ...
  async function loadTweetStatuses(token = '', { renderAfter = true } = {}) {
    try {
      const remote = await readJsonFile(STATUS_PATH, token);
      tweetStatuses = remote.value?.tweets || {};
      if (renderAfter) render();
      return tweetStatuses;
    } catch { return tweetStatuses; }
  }

  async function syncOnReturn() {
    if (foregroundSyncing || document.visibilityState === 'hidden') return;
    foregroundSyncing = true;
    try {
      await loadRemoteState({ renderAfter: true });
      await loadTweetStatuses('', { renderAfter: true });
    } finally { foregroundSyncing = false; }
  }
  ...
  async function saveDeletedIds(mutator, message) {
    ...
    try {
      ...
      await githubRequest(`/contents/${DELETED_PATH}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message, content: encodeBase64Utf8(content), sha: latest.sha, branch: BRANCH }) });
      deletedIds = ids;
      selectedIds.clear();
      els.deletedCount.textContent = deletedIds.size.toLocaleString('ja-JP');
      setSync('保存済み', 'saved');
      ...
      return false;
    } finally { writeInProgress = false; updateBulkToolbar(); }
  }
  ...
  async function startCheck(ids) {
    ...
    try {
      await githubRequest(`/actions/workflows/${encodeURIComponent(WORKFLOW_PATH)}/dispatches`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ref: BRANCH, inputs: { tweet_ids: ids.join(',') } }) });
      for (let attempt = 0; attempt < 24; attempt += 1) {
        await new Promise((resolve) => setTimeout(resolve, attempt === 0 ? 7000 : 5000));
        const statuses = await loadTweetStatuses(getToken(), { renderAfter: false });
    ...
    } finally { checkInProgress = false; els.checkVisible.disabled = false; }
  }
  ...
  [els.type, els.topic, els.reply, els.sort].forEach((element) => element.addEventListener('change', () => applyFilters()));
  els.reset.addEventListener('click', () => { els.type.value = 'all'; els.topic.value = 'all'; els.reply.value = 'all'; els.sort.value = 'new'; applyFilters(); });
  ...
  els.selectVisible.addEventListener('click', () => { filtered.slice(0, visibleCount).forEach((tweet) => selectedIds.add(tweet.id)); render(); });
  els.clearSelected.addEventListener('click', () => { selectedIds.clear(); render(); });
  els.deleteSelected.addEventListener('click', () => { const ids = [...selectedIds]; if (ids.length) saveDeletedIds((set) => ids.forEach((id) => set.add(id)), `Exclude ${ids.length} selected tweets`); });
  els.list.addEventListener('change', (event) => { const input = event.target.closest('[data-select-id]'); if (!input) return; input.checked ? selectedIds.add(input.dataset.selectId) : selectedIds.delete(input.dataset.selectId); updateBulkToolbar(); });
  els.list.addEventListener('click', (event) => { const check = event.target.closest('[data-check-id]'); if (check) { startCheck([check.dataset.checkId]); return; } const remove = event.target.closest('[data-delete-id]'); if (remove) saveDeletedIds((set) => set.add(remove.dataset.deleteId), `Exclude tweet ${remove.dataset.deleteId}`); });
  ...
  els.deletedList.addEventListener('click', (event) => { const restore = event.target.closest('[data-restore-id]'); if (restore) saveDeletedIds((set) => set.delete(restore.dataset.restoreId), `Restore tweet ${restore.dataset.restoreId}`).then(renderDeleted); });
  ...
  els.clearToken.addEventListener('click', () => { setToken(''); els.githubToken.value = ''; els.syncMessage.textContent = 'この端末からトークンを削除しました。'; setSync('未設定', 'unset', 'GitHub同期：閲覧のみ'); });
  document.addEventListener('visibilitychange', () => { if (document.visibilityState === 'visible') syncOnReturn(); });

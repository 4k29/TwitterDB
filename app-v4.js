(() => {
  ...
  const els = {
    ...
    status: $('status'), list: $('tweetList'), loadMore: $('loadMore'), showDeleted: $('showDeleted'),
    deletedCount: $('deletedCount'), deletedDialog: $('deletedDialog'), closeDeleted: $('closeDeleted'),
    deletedQuery: $('deletedQuery'), deletedList: $('deletedList'), syncSettings: $('syncSettings'),
    syncDialog: $('syncDialog'), syncForm: $('syncForm'), closeSync: $('closeSync'), githubToken: $('githubToken'),
    saveToken: $('saveToken'), clearToken: $('clearToken'), syncMessage: $('syncMessage')
  };
  ...
  function tweetRow(tweet) {
    ...
    return `<article class="tweet-row" data-id="${escapeHtml(tweet.id)}"><div class="tweet-date"><b>${escapeHtml(date)}</b>${escapeHtml(time)}</div><div class="tweet-body"><a class="tweet-text" href="${escapeHtml(tweet.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(tweet.text)}</a><div class="badges"><span class="badge">${typeLabel(tweet.type)}</span>${mentionBadges}</div></div><div class="metric likes"><span>いいね</span>${tweet.likes.toLocaleString('ja-JP')}</div><div class="metric reposts"><span>リポスト</span>${tweet.reposts.toLocaleString('ja-JP')}</div><button class="delete-button" type="button" data-delete-id="${escapeHtml(tweet.id)}">削除</button></article>`;
  }

(() => {
  ...
  els.reset.addEventListener('click', () => {
    els.type.value = 'all'; els.reply.value = 'all'; els.sort.value = 'new'; applyFilters();
  });
  ...
  els.deleteSelected.addEventListener('click', () => {
    ...
    if (ids.length) saveDeletedIds((set) => ids.forEach((id) => set.add(id)), `Exclude ${ids.length} selected tweets`);
  });
  ...
  els.list.addEventListener('click', (event) => {
    ...
    if (remove) saveDeletedIds((set) => set.add(remove.dataset.deleteId), `Exclude tweet ${remove.dataset.deleteId}`);
  });
  ...
  els.deletedList.addEventListener('click', (event) => {
    const restore = event.target.closest('[data-restore-id]');
    if (restore) saveDeletedIds((set) => set.delete(restore.dataset.restoreId), `Restore tweet ${restore.dataset.restoreId}`).then(renderDeleted);
  });

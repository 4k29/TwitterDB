(() => {
  ...
  async function readJsonFile(path, token = getToken()) {
  ...
  async function loadRemoteState() {
    ...
    try {
      ...
      updateDeletedCount();
      updateKeptCount();
      setSync('接続済み', 'connected', '非公開データ：接続済み');
    } catch (error) {
      setSync('読込失敗', 'error', `削除状態を読み込めませんでした：${error.message}`);
    }
  }

  async function loadTweetStatuses(token = getToken()) {
    try {
  ...
  async function waitForPublishedPurge(updatedAt) {
    for (let attempt = 0; attempt < 36; attempt += 1) {
      try {
        const published = (await readJsonFile(PURGE_RESULT_PATH)).value;
  ...
  async function verifyAndSaveToken(event) {
    ...
    try {
      ...
      setToken(token);
      els.syncMessage.textContent = '接続できました。非公開データを読み込み直します。';
      setSync('接続済み', 'connected');
      const destination = new URL(location.href).searchParams.get('return');
      location.replace(destination === 'analytics' ? './analytics/' : destination === 'reviews' ? './analytics/reviews.html' : './');
    } catch (error) {
  ...
  async function init() {
    ...
    if (!allTweets.length) {
      els.status.hidden = false;
      els.status.innerHTML = '<strong>非公開の投稿データを読み込めませんでした。</strong><br>同期設定からGitHubトークンを確認してください。';
      return;
    }
    await loadRemoteState();
  ...
  els.clearToken.addEventListener('click', () => {
    window.PrivateTwitterDB?.clearLocalData();
    els.githubToken.value = '';
    els.syncMessage.textContent = 'この端末からトークンとキャッシュを削除しました。';
    setSync('未設定', 'unset', '非公開データ：認証が必要です');
    location.reload();
  });
